...
class BancoDeDados:
    ...
    def CriaPessoa(obj):
        with app.app_context():
            ...
            nova_pessoa = Pessoa(
                nome=obj["nome"], email=obj["email"], senha=hash_senha, tipo=obj["tipo"]
            )
    ...
    def GetEstilosMusicais(id_demanda):
        with app.app_context():
            ...
            for estilo in estilos:
                estilos_musicais.append(
                    EstiloMusical.query.with_entities(EstiloMusical.nome)
                    .filter_by(id=estilo.estilo_musical)
                    .first()[0]
                )
    ...
    def GetMusicos(id_demanda):
        with app.app_context():
            return [
                id[0]
                for id in Match.query.with_entities(Match.id_musico)
                .filter_by(id_demanda=id_demanda)
                .all()
            ]
    ...
    def EnviaMensagem(obj):
        with app.app_context():
            mensagem = Mensagem(
                match=obj["match"], dono=obj["dono"], mensagem=obj["mensagem"]
            )
    ...
    def GetChat(id_match):
        with app.app_context():
            return sorted(
                list(Mensagem.query.filter_by(match=id_match).all()),
    ...
    def GetImagemPerfil(dono):
        with app.app_context():
            return Imagem.query.filter_by(
                dono=dono, tipo_foto=TipoFotoEnum.PERFIL
            ).first()
